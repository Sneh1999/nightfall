...
contract NFTokenShield is Ownable, MerkleTree {
    ...
    */
    function mint(bytes32 tokenContractAddress, uint256[] calldata _proof, uint256[] calldata _inputs, uint256 _tokenId, bytes32 _commitment) external {
    ...
    */
    function burn(bytes32 tokenContractAddress, uint256[] memory _proof, uint256[] memory _inputs, bytes32 _root, bytes32 _nullifier, uint256 _tokenId, uint256 _payTo) public {
        ...
        // Check that the publicInputHash equals the hash of the 'public inputs':
        bytes31 publicInputHash = bytes31(bytes32(_inputs[0])<<8);
        bytes31 publicInputHashCheck = bytes31(sha256(abi.encodePacked(tokenContractAddress, _root, _nullifier, _tokenId, _payTo))<<8);
        require(publicInputHashCheck == publicInputHash, "publicInputHash cannot be reconciled");
